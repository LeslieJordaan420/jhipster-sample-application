...
import za.co.signio.config.KafkaProperties;

class SameApplicationKafkaResourceIT {
    ...
    void setup() {
        ...
        SameApplicationKafkaResource kafkaResource = new SameApplicationKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/same-application-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/same-application-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
